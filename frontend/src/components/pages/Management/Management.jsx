                            ...
                            <tbody>
                                {users.map(user => (
                                    <tr key={user._id}>
                                        ...
                                        <td>{user.cpf}</td>
                                        <td>
                                            <button onClick={(e) => { openModal(user._id) }} id={styles.btn_show_vehicle}>&nbsp;<BsEyeFill style={{ fontSize: "24px" }} /></button>
                                            &nbsp;
                                            <button onClick={(e) => { navigateToRegisterVehiclePage(user._id) }} id={styles.btn_add_vehicle}>&nbsp;<CgPlayListAdd style={{ fontSize: "25px" }} /></button>
                                        </td>
                                        <td>
                                            <button onClick={(e) => { handleDeleteUser(user._id) }} id={styles.btn_delete_user}><MdDelete /></button>
                                            &nbsp;
                                            <button onClick={(e) => { navigateToEditPage(user._id) }} id={styles.btn_edit_user}><FaUserEdit style={{ color: "green", fontSize: "25px" }} /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
